...
function loadMajors() {
    $sql = "SELECT majorcategory.majorName from majorcategory WHERE majorcategory.status=1";
    $con = connect();
    ...
    for ($a = 0; $a < $result->num_rows; $a++) {
        $result->data_seek($a);
        $dat = $result->fetch_assoc()['majorName'];
        $fb = $fb . '<p class="majorCategory" onmouseover="loadData(this.innerHTML)">' . $dat . '<i class="pull-right fa fa-caret-right"></i></p>';
    }
    echo $fb;
}
...
//            WHERE majorcategory.majorName='$majorName'".'\'';
    ...
    for ($a = 0; $a < $r1->num_rows; $a++) {
        ...
        for ($b = 0; $b < $r2->num_rows; $b++) {
            ...
            if (($b + 1) === $r2->num_rows) {
                $minors = $minors . '<a href="viewProduct?q=' . $minorName . '&t=m23">' . $minorName . '</a>';
            } else {
                $minors = $minors . '<a href="viewProduct?q=' . $minorName . '&t=m23">' . $minorName . '</a> | ';
            }
        }
        $all = $all . '<h4><a href="viewProduct?q=' . $catName . '&t=c12">' . $catName . '</a></h4><p>' . $minors . '</p>';
        $minors = '';
    }
    echo $all;
//    echo $sql1;
...
function loadMajorGroupData() {
    ...
    for ($a = 0; $a < $r1->num_rows; $a++) {
        ...
        $fb = $fb . '<a class="col-sm-3" href="viewProduct?q=' . $name . '&t=m23">
...
//item
    ...
    for ($b = 0; $b < $rows2; $b++) {
        ...
        $fb = $fb . '<p><a href="viewProduct?q=' . $catName . '&t=c12"> > ' . $catName . ' </a></p>';
    }
...
//    $number = 9;
    ...
    for ($a = 0; $a < $number; $a++) {
        if ($a === 4) {
            $fb .= '<li><a onclick="pagerClicked(this.innerHTML,this)"><i class="fa fa-chevron-right"></i></a></li>';
            break;
        }
        $fb .= '<li><a onclick="pagerClicked(this.innerHTML,this)">' . ($a + 1) . '</a></li>';
    }
    echo $fb;
}
...
//    $number = 9;
    ...
    for ($a = 0; $a < $number; $a++) {
        if ($a === 0) {
            $fb = '<li><a onclick="pagerClicked(this.innerHTML,this)"><i class="fa fa-chevron-left"></i></a></li>';
        }
        if ($a === 4) {
            $fb = $fb . '<li><a onclick="pagerClicked(this.innerHTML,this)"><i class="fa fa-chevron-right"></i></a></li>';
            break;
        }
        $fb = $fb . '<li><a onclick="pagerClicked(this.innerHTML,this)">' . ($a + ($lastItem + 1)) . '</a></li>';
    }
    echo $fb;
}
...
//$fb = '<li><a onclick="pagerClicked(this.innerHTML,this)"><i class="fa fa-chevron-left"></i></a></li>' . $fb;
        ...
        $fb = '<li><a onclick="pagerClicked(this.innerHTML,this)">' . ($lastItem - ($a + 1)) . '</a></li>' . $fb;
    }
    echo $fb;
}
...
function loadMorePicDivData($name) {
    ...
    for ($a = 0; $a < $rows; $a++) {
        $r->data_seek($a);
        $fb .= '<div class="col-sm-4"><img src="../productImages/' . $r->fetch_assoc()['imageName'] . '" alt="' . $r->fetch_assoc()['itemName'] . '"></div>';
    }
    echo $fb;
}
...
function loadMoreProductsDivDatapD() {
    ...
    for ($a = 0; $a < $rows; $a++) {
        ...
        $fb .= '<a href="viewProduct?q=' . $name . '&t=m23" class="col-sm-3">
                    <img src="../productImages/' . $pic . '" alt="Garlado Limited"/>
                    <p>' . $name . '</p>
                </a>';
        $name = '';
        $pic = '';
