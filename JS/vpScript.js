...
/*  loadFeatures(q, t);   */
    ...
    $('#sortFilter select').change(function () {
        var i = $('#sortFilter span:eq(0)');
        var from = $('.price-range-wrap .price-input input:eq(0)').val();
        var to = $('.price-range-wrap .price-input input:eq(1)').val();
        // var from = $('#priceFrom').val();
        // var to = $('#priceTo').val();
        var filterName = $(this).val();
        // alert(from+' '+to+' '+i.html()+' '+filterName);
        sortFilter(from, to, q, t, filterName, i);
    });

    //**********price Range Change***********/
    $('.price-range-wrap button').click(function(){
        var i = $(this);
        var from = $('.price-range-wrap .price-input input:eq(0)').val();
        var to = $('.price-range-wrap .price-input input:eq(1)').val();
        // var from = $('#priceFrom').val();
        // var to = $('#priceTo').val();
        var filterName = $('#sortFilter select').val();
        // alert(from+' '+to+' '+i.html()+' '+filterName);
        sortFilter(from, to, q, t, filterName, i);
    });
});